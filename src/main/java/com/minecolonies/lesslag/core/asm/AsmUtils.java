...
public class AsmUtils
{
    ...
    public static InsnList cloneList(InsnList list)
    {
        ...
        while (it.hasNext())
        {
            clone.add(it.next().clone(Collections.<LabelNode, LabelNode>emptyMap()));
        }
    ...
    public static Field changeFieldAccess(Class<?> clazz, String fieldName, String srgName, boolean silenced)
    {
        try
        {
            Field f = clazz.getDeclaredField((!Boolean.valueOf(Launch.blackboard.get("fml.deobfuscatedEnvironment").toString())) ? srgName : fieldName);
    ...
    public static Method changeMethodAccess(Class<?> clazz, String methodName, String srgName, boolean silenced, Class<?>... params)
    {
        try
        {
            Method m = clazz.getDeclaredMethod((!Boolean.valueOf(Launch.blackboard.get("fml.deobfuscatedEnvironment").toString())) ? srgName : methodName, params);
